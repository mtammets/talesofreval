...
const DEFAULT_SITE_EMAIL_TEMPLATES = Object.freeze({
...
This message includes a copy of the email sent to ${MAIL_TO} regarding your most recent contact us message.
...
const normalizeSiteEmailTemplates = (templates = {}, legacyFreeTourTemplates = {}) => ({
...
const createLinkifiedParagraph = (value) =>
  escapeHtml(value).replace(
    /(https?:\/\/[^\s<]+)/gi,
    (url) => `<a href="${escapeHtml(url)}" rel="noopener noreferrer" target="_blank">${escapeHtml(url)}</a>`
  );
